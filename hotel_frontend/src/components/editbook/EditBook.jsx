...
function EditBook() {
  ...
  useEffect(() => {
    // Fetch the booking data using the bookingId
  ...
  const fetchBookingData = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/bookings/view/${id}/`);
      if (response.ok) {
  ...
  const deleteBook = () => {
    // Send a DELETE request to the backend to delete the booking
    fetch(`http://127.0.0.1:8000/bookings/${bookingId}/delete`, {
      method: 'DELETE',
      headers: {
  ...
  const handleSubmit = (e) => {
    ...
    // Send a PUT request to update the booking on the backend
